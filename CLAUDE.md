...
- [x] **Threads 채널 전용 문체 강화(NotebookLM/YouTube/Thumbnail은
      ...
      확인).
- [x] **NotebookLM 채널 영상 내레이션 문체 강화(구조/Fact Grounding은
      미변경, Threads/YouTube/Thumbnail도 미변경).** 9구간 구조와
      Fact Grounding 판정 기준은 그대로 두고 시스템 프롬프트만
      다시 썼다. Hook은 "핵심 숫자/사건 → 의외의 시장 반응 → 질문"
      순서를 우선하도록 예시와 함께 명시(첫 20~30초, 말하는 속도
      기준 대략 50~80자 - 기존에 이 파일이 써 온 "4~6분=600~950자"
      환산 비율과 일치시켰다). 어려운 개념은 용어부터 정의하지 않고
      "쉬운 현상/비유 → 용어" 순서로 설명하도록 나쁜 예/좋은 예를
      추가했다. 결론은 단순 요약이 아니라 "그래서 앞으로 무엇을
      확인해야 하는가"에 집중하도록 바꾸고, 마지막 30초는 확인할
      지표(update_triggers에 실제로 있는 것만)와 그 지표가 어느
      방향이면 해석이 강화/약화되는지를 함께 설명하도록 명시했다.
      한 문장 하나의 정보, 두세 문장마다 줄바꿈, 연결어("그래서"/
      "여기서 중요한 건"/"그런데") 반복 금지, 보고서체 금지, 저확신도
      fact는 "일부 시장에서는 ~라고 봅니다"/"한 가지 해석은 ~입니다"로
      제한하도록 지침을 강화했다. 분량 목표를 "4~7분=600~1,100자"에서
      "4~6분=600~950자 우선 목표, 정보 부족 시 억지로 7분 채우지
      않음"으로 조정. Threads/YouTube와 같은 원칙으로
      `_check_no_internal_ids_leaked()`를 추가해 title/hook/script/
      chapters에 `fact_004` 같은 내부 식별자가 노출되면
      `NotebookLmInternalIdLeakError`로 막는다(다른 채널과 로직을
      공유하지 않고 이 파일에 독립적으로 구현). 테스트:
      `tests/test_notebooklm_style.py`(25개, hook 구조/개념 설명
      순서/문단 길이/연결어·보고서체 금지/저확신도 표현/결론·마지막
      30초 구조/내부 id 노출 차단/기존 Fact Grounding 유지/분량 확인).
